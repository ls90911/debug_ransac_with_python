...
#include "RANSAC.h"
#include "pprz_matrix_decomp_float.h"
...
void RANSAC_linear_model(int n_samples, int n_iterations, float error_threshold, float *targets, int D,
                         float (*samples)[2], uint16_t count, float *params, float *fit_error)
{

  uint8_t D_1 = D + 1;
  float err;
  float* errors = (float*) alloca(n_iterations*sizeof(float));
  int* indices_subset = (int*) alloca(n_samples*sizeof(int));
  float* subset_targets = (float*) alloca(n_samples*sizeof(float));
  float** subset_samples = (float**) alloca(n_samples*D*sizeof(float));
