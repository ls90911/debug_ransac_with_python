...
#define MAKE_MATRIX_PTR_S(_ptr, _mat) \
  { \
    int __i; \
    for (__i = 0; __i < MAX_SAMPLES; __i++) { _ptr[__i] = &(_mat[__i][0]); } \
  }

#define MAKE_MATRIX_PTR_D1(_ptr, _mat) \
  { \
    int __i; \
    for (__i = 0; __i < (VECTOR_DIMENSION+1); __i++) { _ptr[__i] = &(_mat[__i][0]); } \
  }
