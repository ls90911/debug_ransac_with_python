 ...
 */
void fit_linear_model(float targets[MAX_SAMPLES], int Dimension, float samples[MAX_SAMPLES][1], uint16_t count, bool use_bias, float *params,
                      float *fit_error)
{
  printf("[pprz_matrix] 1\n");
  ...
  float _AA[MAX_SAMPLES][VECTOR_DIMENSION+1];
  float _targets_all[MAX_SAMPLES][1];
  float * AA[MAX_SAMPLES];
  float * targets_all[MAX_SAMPLES];
  MAKE_MATRIX_PTR_S(AA, _AA);
  MAKE_MATRIX_PTR_S(targets_all, _targets_all);
  ...
  for (sam = 0; sam < count; sam++) {
    ...
    if (use_bias) {
      AA[sam][Dimension] = 1.0f;
    } else {
      AA[sam][Dimension] = 0.0f;
    }
  ...
  {
  float _parameters[VECTOR_DIMENSION+1][1];
  float * parameters[VECTOR_DIMENSION+1];
  float * v[VECTOR_DIMENSION+1];
  float w[MAX_SAMPLES];
  float _v[VECTOR_DIMENSION+1][VECTOR_DIMENSION+1];
  MAKE_MATRIX_PTR_D1(parameters, _parameters);
  MAKE_MATRIX_PTR_D1(v, _v);
  ...
  {
  float _bb[MAX_SAMPLES][1];
  float *bb[MAX_SAMPLES];
  float *C[MAX_SAMPLES];
  float _C[MAX_SAMPLES][1];
  MAKE_MATRIX_PTR_S(bb, _bb);
  MAKE_MATRIX_PTR_S(C, _C);
